...
def getInput(infile: str) -> T.Generator:
    return (v.rstrip("\n") for v in open(infile, "r"))


def calculate_score(board_: list[list[tuple[int, bool]]], lastnum: int):
    return sum((s[0] for r in board_ for s in r if s[1] is False)) * lastnum
...
if __name__ == "__main__":
    ...
    for line in input[1:]:
        if line == "":
            current_board: list[list[tuple[int, bool]]] = []
            boards.append((current_board, False))
            continue
        current_board.append([(int(n), False) for n in line.strip().split()])
    else:
    ...
    for num in calls:
        all_complete = True  # Unless we find an incomplete board
        for board_num, (board, complete) in enumerate(boards):
            if complete:
                continue
            all_complete = False
            for row in board:
                ...
                for square in row:
                    if square[1] is False:
                        break
                else:
                    complete = True
                    boards[boards.index((board, False))] = (board, True)
                    results.append((board_num, calculate_score(board, num)))

            if not complete:
                for col in range(len(board[0])):
                    for row in board:
                        if row[col][1] is False:
                            break
                    else:
                        boards[boards.index((board, False))] = (board, True)
                        results.append(
                            (board_num, calculate_score(board, num))
                            )
        if all_complete:
            break

    print(f"Board #{results[-1][0] + 1}: {results[-1][1]}")
