...
Number of fish after 80 days: 360610
...
if __name__ == "__main__":
    ...
    table = bytes.maketrans(
        bytes([0, 1, 2, 3, 4, 5, 6, 7, 8] + [*range(10, 256)]),
        bytes([6, 0, 1, 2, 3, 4, 5, 6, 7] + [*range(10, 256)])
        )

    fishes = bytes([3, 4, 3, 1, 2])
    # fishes = bytes((
    #     int(i)
    ...
    for day in range(1, NUMBER_OF_DAYS + 1):
        ...
        fishes = fishes.translate(table) + bytes([8]) * fishes.count(0)
        # print(
        #     f"After {day: >2d} {days} "
        #     f"{','.join((str(fish) for fish in fishes))}"
        #     )
        print(f"After {day: >2d} {days} {len(fishes)}")
